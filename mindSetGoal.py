...
from io import BytesIO
import time
import openpyxl  # Fix for Excel file conversion
...
if upload_files:
    for file in upload_files:
        ...
        # Data Visualization
        with st.expander("📊 **Data Visualization**"):
            chart_type = st.selectbox("Choose Chart Type", ["Bar Chart", "Line Chart", "Pie Chart"], key=file.name)
        ...
        with st.expander("🔄 **Convert & Download File**"):
            ...
            if st.button(f"💾 Convert & Download {file.name}"):
                ...
                st.success(f"🎉 {file_name} has been successfully converted and is ready for download!")
